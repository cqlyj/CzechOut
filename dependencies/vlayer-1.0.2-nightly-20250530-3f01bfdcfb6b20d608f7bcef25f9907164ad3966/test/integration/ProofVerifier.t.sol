...
interface PinnedSimpleProver {
    function balance(
        address _owner
    ) external returns (Proof memory, address, uint256);
}

contract FakeProofVerifierUnderTest is FakeProofVerifier {
    constructor()
        FakeProofVerifier(new Repository(address(this), address(this)))
    {
        IMAGE_ID_REPOSITORY.addImageIdSupport(
            ProofFixtures.FIXED_CALL_GUEST_ID
        );
    }
}
...
contract Groth16ProofVerifierUnderTest is Groth16ProofVerifier {
    constructor()
        Groth16ProofVerifier(new Repository(address(this), address(this)))
    {
        IMAGE_ID_REPOSITORY.addImageIdSupport(
            ProofFixtures.FIXED_CALL_GUEST_ID
        );
...
contract PinnedProofVerifer_Tests is Test {
    ...
    function test_canVerifyFakeProof() public {
        vm.setBlockhash(
            ...
            ProofFixtures.FIXED_FAKE_SETTLE_BLOCK_HASH
        );
    ...
    function test_canVerifyGroth16Proof() public {
        vm.setBlockhash(
            ProofFixtures.FIXED_SETTLE_BLOCK_NUMBER,
            ProofFixtures.FIXED_GROTH16_SETTLE_BLOCK_HASH
        );
        vm.chainId(ProofFixtures.FIXED_SETTLE_CHAIN_ID);
    ...
    function _fromFuzzable(
        FuzzableProof memory proof
    ...
    function _fromFuzzable(
        FuzzableSeal memory seal
    ) internal pure returns (Seal memory) {
        return
            Seal(
    ...
    function _arbitraryProof(
        Proof memory originalProof,
        Proof memory randomProof
    ) internal returns (Proof memory, bytes32) {
        Proof memory arbitraryProof;
        arbitraryProof.seal.verifierSelector = _randomBool()
            ? randomProof.seal.verifierSelector
            : originalProof.seal.verifierSelector;
        arbitraryProof.seal.seal = _randomBool()
            ? randomProof.seal.seal
            : originalProof.seal.seal;
        arbitraryProof.seal.mode = _randomBool()
            ? randomProof.seal.mode
            : originalProof.seal.mode;
        arbitraryProof.callGuestId = _randomBool()
            ? randomProof.callGuestId
            : originalProof.callGuestId;
        ...
        arbitraryProof.callAssumptions.settleChainId = _randomBool()
            ? randomProof.callAssumptions.settleChainId
            : originalProof.callAssumptions.settleChainId;
        ...
        arbitraryProof.callAssumptions.settleBlockHash = _randomBool()
            ? randomProof.callAssumptions.settleBlockHash
            : originalProof.callAssumptions.settleBlockHash;
        return (
            arbitraryProof,
            ProofFixtures.journalHash(
                randomProof.callAssumptions,
                ProofFixtures.FIXED_OWNER,
                ProofFixtures.FIXED_BALANCE
            )
        );
    }
    ...
    function testFuzz_cannotVerifyManipulatedGroth16Proof(
        ...
        (
            Proof memory arbitraryProof,
            bytes32 arbitraryJournalHash
        ) = _arbitraryProof(proof, randomProof);
        vm.assume(
            keccak256(abi.encode(arbitraryProof)) !=
...
library ProofFixtures {
    bytes32 public constant FIXED_CALL_GUEST_ID =
        bytes32(
            0x27af6fa6848425013a1e3829d0004247ee6123bd4bafb264546627d19235096b
        );
    ...
    bytes32 public constant FIXED_GROTH16_SETTLE_BLOCK_HASH =
        bytes32(
            0x108374c602edb7492fcb83f266c022056980cc292b6c75c6cbdcd35c0ddb5020
        );
    bytes32 public constant FIXED_FAKE_SETTLE_BLOCK_HASH =
        bytes32(
            0x7b7f1af934e42703670329a42b6df211fbafa0c8b2a6805816f9db6e9ea47dc0
        );

    address public constant FIXED_OWNER =
    ...
    function fakeProofFixture() public pure returns (Proof memory, bytes32) {
        bytes32[8] memory sealBytes = [
            bytes32(
                0x437de4f7c2e014d2062a44164ac4b7b3613d4f02da31293f3a33215d06a2d981
            ),
            bytes32(
    ...
    function groth16ProofFixture() public pure returns (Proof memory, bytes32) {
        bytes32[8] memory sealBytes = [
            bytes32(
                0x2d1c27ce49d0d02887b2389952390eeb0aa01c385668de1321256b11ac94fa9b
            ),
            bytes32(
                0x2f53731dfa5d3e861335c39417fd033522d31210220581878065d0ef278e97be
            ),
            bytes32(
                0x062f43a31905646a546681cbca0cdd9b0fbf123074f80cc0fdc0292fdb6b70ca
            ),
            bytes32(
                0x194711a30a0493b3a0d5a8cc2f60c83ca123816c0ec2cee72215bf1b8ee5ba85
            ),
            bytes32(
                0x0a7bdccce263f3c88a2068d6679a3fb134e6ba798fb1370dec93492c4cec9a91
            ),
            bytes32(
                0x176903a80c73d733739f8b75798625a770811d448d598507eb281c6648f23843
            ),
            bytes32(
                0x2ee28cf9759f80b1c4dea8b7727e6e2e8c08c5676c7aced91666cd7043a11ffe
            ),
            bytes32(
                0x1db943bcdac6ed047178dce405f09b0d48e23d447353c2fe9557d2eb20bfb42f
            )
        ];
        ...
        return
            generateProof(seal, ProofFixtures.FIXED_GROTH16_SETTLE_BLOCK_HASH);
    }
    ...
    function generateProof(
        ...
        return (
            proof,
            journalHash(callAssumptions, FIXED_OWNER, FIXED_BALANCE)
        );
    }
